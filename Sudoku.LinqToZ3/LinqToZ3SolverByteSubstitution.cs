...
{
    public class LinqToZ3SolverByteSubstitution : ISudokuSolver
    {

        private static readonly int[] Indices = Enumerable.Range(0, 9).ToArray();
        // Utilisation du type Byte vector qui a été implémenter directement dans les switchs de la bibliothéques LinqTo Z3 et utilisation ici
        private static Byte[][] CellVariables;
        ...
        public static Theorem<SudokuGrid> CreateTheorem(Z3Context context, SudokuGrid s)
        {
            ...
            for (int i = 0; i < 9; i++)
            {
                for (int j = 0; j < 9; j++)
                {
                    var idxi = i;
                    var idxj = j;
                    // De meme pour la définition des cellules utilisations du type Byte pour l'attribution de la valeur chaque cellule
                    var cellValue = (Byte)s.Cells[idxi][idxj];
                    if (s.Cells[i][j] != 0)
                    {
                        // ICI UTILISATION DE LA SUBSITUTION AVEC L'API BYTE
        ...
        public static Theorem<SudokuGrid> Create(Z3Context ctx)
        {
            var sudokuTheorem = ctx.NewTheorem<SudokuGrid>();
