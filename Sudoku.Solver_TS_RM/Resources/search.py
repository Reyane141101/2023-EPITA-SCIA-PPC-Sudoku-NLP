...
class Node:
    ...
    def child_node(self, problem, action):
        ...
        next_node = Node(next_state, self.parent, action, problem.path_cost(self.path_cost, self.state, action, next_state))
        return next_node
...
def depth_limited_search(problem, limit=60):
    """[Figure 3.17]"""
...
def compare_graph_searchers():
    ...
    # compare_searchers([NQueensProblem(4),NQueensProblem(8)], header = ['Searcher', '4 Queens', '8 Queens'])
    a = [i for i in range(0,9)]
    random.shuffle(a)

    compare_searchers([EightPuzzle(a)], header = ['Searcher', 'A'])
