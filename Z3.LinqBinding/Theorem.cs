 ...
 /// <summary>
        ...
        private void AssertConstraints<T>(Context context, Object solverOrOptimizer, Environment environment)
        {
            ...
            foreach (var constraint in constraints)
            {
                ...
                BoolExpr c;
                // Ajout d'un switch pour pouvoir discerner la substituion avec les byte et int
                switch(constraint.Type)
                {
                     /*case ConstraintType.Int:
                        c = ApplyIntSubstitution(context, environment, flattenedBody, constraint.Parameters[0]);
                        break;*/
                    /*case ConstraintType.Byte:
                        c = ApplyByteSubstitution(context, environment, flattenedBody, constraint.Parameters[0]);
                        break;*/
                     default:
                      c = (BoolExpr)Visit(context, environment, flattenedBody, constraint.Parameters[0]);
                      break;
                }
        ...
        private Expr VisitConstantValue(Context context, Object val)
        {
            ...
            {
                case TypeCode.Byte:
                    return context.MkBV(Convert.ToByte(val), 8);
                case TypeCode.Int16:
                case TypeCode.Int32:
                case TypeCode.Int64:
